...
export const fetchSurveyData = (surveyId: number = 1, matrixId: number = 1) => {
  const groupsFound = [];
  let survey = fetchQuestionsBySurveyId(surveyId).then((questionsData) =>
    questionsData
      .map((data) => {
        if (data.type === "matrix") {
          fetchMatrixById(matrixId).then((matrixData) => {
